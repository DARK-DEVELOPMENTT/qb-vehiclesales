...
local function SellData(data, model)
    local vehicleModel = string.lower(GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(PlayerPedId()))))
    if Config.BlackListCars and table.contains(Config.BlackListCars, vehicleModel) then
        QBCore.Functions.Notify("You Can't Sell Imported Cars", "error", 3500)
        return
    end
